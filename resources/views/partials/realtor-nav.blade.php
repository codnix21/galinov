...
<nav class="flex flex-wrap gap-2 mb-8 p-2 bg-white rounded-2xl border border-slate-200 shadow-sm">
    ...
    <a href="{{ route('realtor.kanban') }}" class="px-3 py-2 rounded-xl text-sm {{ request()->routeIs('realtor.kanban*') ? 'bg-brand-100 text-brand-900 font-medium' : 'text-slate-600 hover:bg-slate-50' }}">Воронка</a>
    <a href="{{ route('realtor.analytics') }}" class="px-3 py-2 rounded-xl text-sm {{ request()->routeIs('realtor.analytics') ? 'bg-brand-100 text-brand-900 font-medium' : 'text-slate-600 hover:bg-slate-50' }}">Аналитика</a>
