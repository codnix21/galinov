...
<div class="max-w-3xl mx-auto">
    ...
    <form method="POST" action="{{ route('properties.update', $property) }}" class="card p-8" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-6 mb-6">
            <div>
                ...
                @error('gorod')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500">Подсказки только по населённым пунктам. При смене города уточните улицу заново.</p>
            </div>
            <div>
                ...
                @error('adres_ulitsy')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-gray-500">Только улица и дом (без города). Выберите вариант из подсказки для точки на карте.</p>
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-6 mb-6">
            <div>
                ...
                <input type="number" id="etazh" name="etazh" value="{{ old('etazh', $property->etazh ?? $property->floor) }}" min="1" class="form-input">
                @error('etazh')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                ...
                <input type="number" id="vsego_etazhey" name="vsego_etazhey" value="{{ old('vsego_etazhey', $property->vsego_etazhey ?? $property->total_floors) }}" min="1" class="form-input">
                @error('vsego_etazhey')
                    <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
                @enderror
            </div>
        ...
        <div class="mb-6">
            ...
            <select id="status_obyavleniya" name="status_obyavleniya" required class="form-input select-native">
                ...
                <option value="active" {{ old('status_obyavleniya', $property->status_obyavleniya ?? $property->status) == 'active' ? 'selected' : '' }}>{{ ($property->status_obyavleniya ?? $property->status) === 'active' ? 'Опубликовано (в каталоге)' : 'Отправить на модерацию' }}</option>
                @if(($property->operatsiya ?? $property->operation) == 'sale')
                    <option value="sold" {{ old('status_obyavleniya', $property->status_obyavleniya ?? $property->status) == 'sold' ? 'selected' : '' }}>Продано</option>
                @elseif(($property->operatsiya ?? $property->operation) == 'rent')
            ...
            @error('status_obyavleniya')
                <p class="mt-2 text-sm text-red-600">{{ $message }}</p>
            @enderror
            <p class="mt-2 text-sm text-gray-600">Объявления со статусом "Продано" или "Сдано" не будут отображаться на главной странице</p>
        </div>
